...
public abstract class Command {
	...
	/**
	 * Uďż˝ciďż˝lenie protected static String [] getValues (String line, String [] names) dla komend +INFO Queue 'rekawek', Count '1'
	 ...
	 * Jeďż˝li ktďż˝ryďż˝ z parametrďż˝w nie zostanie znaleziony - zwracam null.
	 * Obsďż˝uguje linie typu: 
	 * EVENT [Agent], Action [SomeNewCall], Agent [301], Exten [222], Queue [rekawek], CallerID [], Uniqueid [1202119281.30662]
	 *  +INFO Queue 'rekawek', Count '1'
	 * @param line zawartoďż˝ďż˝ linii
	 * @param names nazwy parametrďż˝w
	 * @return tablica z wartoďż˝ci dla poszczegďż˝lnych parametrďż˝w
	...
	public static Command factory(String line, String connectionId) {
		...
		Command c;
        CommandFactory commandFactory = new InfoVersionFactory();

        c = commandFactory.factoryInt(line);
		if (c != null) {
			c.connectionId = connectionId;
			return c;
		}
        CommandWelcomeFactory commandWelcomeFactory = new CommandWelcomeFactory();
        c = commandWelcomeFactory.factoryInt(line);
		if (c != null) {
			c.connectionId = connectionId;
			return c;
		}
		c = CommandLoggedIn.factoryInt(line);
		if (c != null) {
			c.connectionId = connectionId;
			return c;
		}
